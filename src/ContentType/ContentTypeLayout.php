...
final class ContentTypeLayout implements ContentTypeInterface
{
    ...
    {
        $layout = $arrData['layout'] ?? null;

        if (!$layout instanceof LayoutModel) {
            return [];
        }

        $arrLayoutData = $layout->row();

        $stringUtil = $this->framework->getAdapter(StringUtil::class);

        return $stringUtil->deserialize($arrLayoutData['cssStyleSelector'] ?? '', true);
    }

    public function isDisabled(array $arrStyle, Request $request): bool
    {
        return (bool) $arrStyle['disableInLayout'];
    }
}
